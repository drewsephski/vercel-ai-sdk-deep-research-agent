...
- **[Neon Postgres](https://neon.tech/)** for serverless PostgreSQL database
...
2. Copy the `example.env.local` file to `.env` and fill in the required environment variables:
   ...
   - Neon Postgres database connection string. Create a free account at [Neon](https://neon.tech/).
3. Copy your project ref from your [Trigger.dev dashboard](https://cloud.trigger.dev/) and add it to the `trigger.config.ts` file.
4. Run database migrations with `npm run db:push` to create the database schema.
5. Run the Next.js server with `npm run dev`.
6. In a separate terminal, run the Trigger.dev dev CLI command with `npx trigger@v4-beta dev` (it may ask you to authorize the CLI if you haven't already).
7. To test your deep research agent, go to `http://localhost:3000` and start researching any topic.
...
- **Database schema**: [`src/db/schema.ts`](src/db/schema.ts) - defines the database schema for users and research sessions using Drizzle ORM.
- **Database client**: [`src/db/index.ts`](src/db/index.ts) - database connection and Drizzle client instance.
...
- [Neon Postgres](https://neon.tech/docs) - Serverless PostgreSQL database
- [Drizzle ORM](https://orm.drizzle.team/docs/overview) - Type-safe SQL toolkit
